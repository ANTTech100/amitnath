...
export default function AllProductsCards() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Step 2: Find the "All Products" template
        console.log("Searching for template named 'All Products'...");
        const allProductsTemplate = templateData.data.find((template) => {
          console.log(
            `Comparing template name: "${template.name}" with "All Products"`
          );
          return template.name === "All Products";
        });

        if (!allProductsTemplate) {
          console.log("Template 'All Products' not found in the data.");
          throw new Error("Template 'All Products' not found");
        }

        console.log("Found Template:", allProductsTemplate);
        const templateId = allProductsTemplate._id;
  ...
  const handleCardClick = (id) => {
    router.push(`/layouts/layoutsix/${id}`);
  };
    ...
    <div className="bg-gradient-to-br from-teal-900 to-gray-800 py-16 px-4 sm:px-6 lg:px-8 min-h-screen">
      <div className="max-w-5xl mx-auto">
        <Head>
          ...
          <meta name="description" content="Explore our All Products pages!" />
        </Head>
