  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Step 5: Initialize showMoreSections state
        const initialShowMore = {};
        filteredContents.forEach((content) => {
          initialShowMore[content._id] = false;
        });
        console.log("Initial Show More Sections State:", initialShowMore);
        setShowMoreSections(initialShowMore);
      } catch (err) {
        ...
        return (
          <div key={content._id}>
            ...
            {contentIndex < contents.length - 1 && (
              <hr className="max-w-6xl mx-auto my-16 border-t border-gray-300" />
            )}
          </div>
        );
      })}
