// pages/VideoTestimonialCards.jsx
"use client";
import { useState, useEffect } from "react";
import { useRouter } from "next/navigation";
import Head from "next/head";

export default function VideoTestimonialCards() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Step 2: Find the "Video Testimonial" template
        console.log("Searching for template named 'Video Testimonial'...");
        const videoTestimonialTemplate = templateData.data.find((template) => {
          console.log(
            `Comparing template name: "${template.name}" with "Video Testimonial"`
          );
          return template.name === "Video Testimonial"; // Updated to "Video Testimonial"
        });

        if (!videoTestimonialTemplate) {
          console.log("Template 'Video Testimonial' not found in the data.");
          throw new Error("Template 'Video Testimonial' not found");
        }

        console.log("Found Template:", videoTestimonialTemplate);
        const templateId = videoTestimonialTemplate._id;
  ...
  const handleCardClick = (id) => {
    ...
    router.push(`/layouts/layouteight/${id}`); // Updated to layouteight
  };
    ...
    <div className="bg-gradient-to-br from-teal-900 to-gray-800 py-16 px-4 sm:px-6 lg:px-8 min-h-screen">
      <div className="max-w-5xl mx-auto">
        <Head>
          ...
          <meta name="description" content="Explore our Video Testimonials!" />
        </Head>
