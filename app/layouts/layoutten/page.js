  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log("Searching for template named 'Basic'...");
        const paymentPageTemplate = templateData.data.find((template) => {
          console.log(
            `Comparing template name: "${template.name}" with "Basic"`
          );
          return template.name === "Basic";
        });

        if (!paymentPageTemplate) {
          console.log("Template 'Basic' not found in the data.");
          throw new Error("Template 'Basic' not found");
        }

        console.log("Found Template:", paymentPageTemplate); // Fixed
        const templateId = paymentPageTemplate._id; // Fixed
    ...
    <div className="bg-gradient-to-br from-blue-50 to-gray-100 py-16 px-4 sm:px-6 lg:px-8 min-h-screen">
      <div className="max-w-5xl mx-auto">
        <Head>
          ...
          <meta name="description" content="Explore our Payment Pages!" />
        </Head>
